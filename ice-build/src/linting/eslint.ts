 ...
 */
export function canUseEslint(): boolean {
  try {
 ...
 */
export function hasEslintConfig(projectDir: string): boolean {
  const configFiles = [
    'eslint.config.js',
    '.eslintrc.js',
    '.eslintrc.json',
 ...
 */
export async function lintFiles(): Promise<{errorCount: number, warningCount: number}> {
